...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    let channel: ReturnType<typeof import("@supabase/supabase-js").createClient> extends never
      ? never
      : any;
    (async () => {
      const { supabase } = await import("@/integrations/supabase/client");
      channel = supabase
        ...
        .on(
          ...
          () => {
            toast.info("Pengajuan centang biru baru masuk");
            load();
          },
        )
        .on(
          ...
          () => {
            toast.info("Pengajuan langganan baru masuk");
            load();
          },
        )
        .on(
          "postgres_changes",
          { event: "INSERT", schema: "public", table: "promotion_requests" },
          () => {
            toast.info("Pengajuan promosi baru masuk");
            load();
          },
        )
        .on(
        ...
        .on(
          "postgres_changes",
          { event: "UPDATE", schema: "public", table: "promotion_requests" },
          () => load(),
        )
        .on(
          "postgres_changes",
